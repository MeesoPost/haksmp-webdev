...
async function fetchModrinthData(): Promise<void> {
  try {
    ...
    // Create modpack info from most popular version
    let modpackInfoVersion = latestVersion.version_number;
    let modpackInfoMinecraftVersion = gameVersion;
    let modpackInfoForgeVersion = loaderVersion;

    if (modpacks.length > 0) {
      modpackInfoVersion = modpacks[0].version;
      modpackInfoMinecraftVersion = modpacks[0].gameVersion;
    }

    const modpackInfo: ModpackInfo = {
      name: modpackProject.title,
      version: modpackInfoVersion,
      minecraftVersion: modpackInfoMinecraftVersion,
      forgeVersion: modpackInfoForgeVersion,
